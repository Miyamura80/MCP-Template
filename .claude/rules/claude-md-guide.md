...
1. **Non-obvious** - not derivable from a directory name, a standard tool, or linter/formatter defaults.
...
- Non-standard infra choices (LLM stack, config overlay scheme, anything a reader would assume differently by default).
...
- Restating directory purpose when the name already says it (the database lives in `db/`, tests live in `tests/`, CI workflows in `.github/workflows/`).
...
- Style rules already enforced by the linter/formatter (snake_case, quote style, line length, `list` vs `typing.List`). The `## Code Style` section should just point at the linter config.
