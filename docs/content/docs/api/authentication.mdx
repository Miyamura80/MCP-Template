...
Cursor pagination (rather than `offset`/`page`) keeps results stable while keys
are created or revoked between fetches, so an agent looping through pages never
skips or double-counts a key.
