...
| `402` | `payment_required` / `quota_exceeded` | After reset | Daily quota exhausted — wait for `Retry-After`, or upgrade the plan |
...
1. **Honor `Retry-After` first.** On `429` and `402` the server tells you
   exactly how many seconds to wait. Sleep at least that long before retrying.
...
3. **Cap attempts** (e.g. 5) and surface the final `request_id` to the user.
4. **Idempotency:** writes that must not double-apply accept an
...
def call_with_retry(client: httpx.Client, request: httpx.Request, max_attempts: int = 5):
    for attempt in range(max_attempts):
        ...
        if resp.status_code < 400 or resp.status_code not in RETRYABLE:
            return resp
        # Server-directed wait (429/402) takes priority over backoff.
