...
| `409` | Idempotency-Key in progress | A request with the same key is still running; retry with backoff. A key left in-flight by a crashed request clears after the 24h retention window |
| `422` | Unprocessable | Validation failed, or an `Idempotency-Key` was reused with a different payload |
