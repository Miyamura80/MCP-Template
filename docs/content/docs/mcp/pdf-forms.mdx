...
If the client cannot render MCP Apps but supports elicitation, the ceremony
falls back to a host-native typed-name prompt (`full_name` + `consent`) with
...
the PDF tools as a standalone add-on later means moving the core modules and
replacing that one bridge file.
